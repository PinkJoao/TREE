...
class _TreePageState extends State<TreePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: TextScroll(widget.father != null && widget.father != 'null'
  ...
  Future checkAndGet() async {
    await checkDownload();
    await getLevelButtons();
  }

  addActiveButton(List active, [bool check = false]){
    String itemName = active[1].toString().replaceAll('[', '').replaceAll(', ', ' [');
  ...
  Future<void> getLevelButtons() async {
    ...
    if(widget.father == null || widget.father == 'null' || widget.father == ''){
      for (var active in widget.levels[widget.currentLevel]){
        String itemName = active[1].toString();

        if(!alreadyAdded.contains(itemName)){

          alreadyAdded.add(itemName);
          addActiveButton(active);

        }
      }
    }else{
      List<String> pendingFiles = await getDirectoryFileNames('PENDENTES TREE', downloadDir) ?? [];
      for (var active in widget.levels[widget.currentLevel].where((element) => element[0].toString() == fatherFilters.toString() && !alreadyAdded.contains(element[1].toString()))){
        String itemName = active[1].toString();
        String tag = itemName.split(', ').last.replaceAll(']', '').replaceAll('/', '%');

        if(!alreadyAdded.contains(itemName)){

          alreadyAdded.add(itemName);
          addActiveButton(active, pendingFiles.where((element) => element.split('_').first == tag).isNotEmpty);
